import { Room, Player, PlacedTile, Letter, Move } from "./types";
import {
  createEmptyBoard,
  createEnglishBag,
  dealToSeven,
  scoreSimple,
  scoreWordWithPremiums,
  validateAndScoreAllWords,
  calculateTileBagStats,
  checkGameEnd,
...
function updateTileBagStats(room: Room): void {
  room.tileBagStats = calculateTileBagStats(room);
}

function startPlayerTimer(room: Room, playerId: string): void {
  if (!room.timerActive || !room.timeLimit) return;

  room.currentPlayerStartTime = Date.now();
}

function endPlayerTimer(room: Room, playerId: string): void {
...
function checkTimerForfeits(room: Room): void {
...
function checkAndHandleGameEnd(room: Room): void {
...
async function addMoveToHistory(
  room: Room,
  playerId: string,
  tiles: PlacedTile[],
  validationResult: {
  ...
  const wordDefinitions: { [word: string]: string } = {};
...
export async function placeWord(
  ...
  const room = rooms.get(roomId);
  if (!room) throw new Error("room_not_found");
  if (room.players.length < 2) throw new Error("waiting_for_second_player");
  if (room.currentTurnPlayerId !== playerId) throw new Error("not_your_turn");

  // Check if this is the first move
  const isFirstMove = room.board.every((row) =>
    row.every((cell) => cell.letter === null)
  );
  ...
  const wordFormationValidation = validateWordFormation(
    tiles,
    room.board,
    isFirstMove
  );
  ...
  const validationResult = await validateAndScoreAllWords(
    tiles,
    room.board,
    isValidWordAsync
  );
  ...
  const primaryWord =
    validationResult.words[0]?.word || tiles.map((t) => t.letter).join("");
...
export function swapTiles(
  ...
  if (room.players.length < 2) throw new Error("waiting_for_second_player");
  if (room.currentTurnPlayerId !== playerId) throw new Error("not_your_turn");
  if (room.bag.length < 7) throw new Error("cannot_swap_when_bag_below_7");
  if (!letters.length) throw new Error("no_letters_to_swap");
  ...
  for (const l of letters) {
    ...
    if (idx === -1) throw new Error("missing_letter");
    rackCopy.splice(idx, 1);
  }
  // put letters back and draw same amount
  for (const l of letters) room.bag.unshift(l);
  // draw
  for (let i = 0; i < letters.length && room.bag.length; i++) {
    rackCopy.push(room.bag.pop() as Letter);
  }
  player.rack = rackCopy;
  updateTileBagStats(room);
...
export async function resolveChallenge(
  ...
  const room = rooms.get(roomId);
  if (!room) throw new Error("room_not_found");
  if (!room.challengeMode) throw new Error("challenge_disabled");
  if (!room.pendingMove) throw new Error("no_pending_move");
  // Only opponent can resolve
  const pendingBy = room.pendingMove.byPlayerId;
  if (pendingBy === playerId) throw new Error("cannot_resolve_own_move");

  const mover = room.players.find((p) => p.id === pendingBy)!;

  // Validate all words formed by the pending move
  const validationResult = await validateAndScoreAllWords(
    room.pendingMove.tiles,
    room.board,
    isValidWordAsync
  );
...
export function skipTurn(roomId: string, playerId: string): Room {
  const room = rooms.get(roomId);
  if (!room) throw new Error("room_not_found");
  if (room.players.length < 2) throw new Error("waiting_for_second_player");
  if (room.currentTurnPlayerId !== playerId) throw new Error("not_your_turn");
  if (room.gameEnded) throw new Error("game_ended");
...
export function resignGame(roomId: string, playerId: string): Room {
  const room = rooms.get(roomId);
  if (!room) throw new Error("room_not_found");
  if (room.players.length < 2) throw new Error("waiting_for_second_player");
  if (room.gameEnded) throw new Error("game_ended");
  ...
  const otherPlayer = room.players.find((p) => p.id !== playerId);
  if (!otherPlayer) throw new Error("no_opponent");
