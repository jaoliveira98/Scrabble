...
import { ClientMessage, ServerMessage } from "./game/types";
import {
  createRoom,
  getRoom,
  joinRoom,
...
app.get("/", (_req, res) => {
  res
    .type("text")
    .send(
      "Scrabble server is running. Try GET /healthy or connect via WebSocket."
    );
});
    ...
    try {
      ...
      switch (msg.type) {
        ...
        case "resolve_challenge": {
          try {
            const room = await resolveChallenge(
              msg.roomId,
              clientId,
              msg.action
            );
...
function broadcastRoom(roomId: string) {
  ...
  for (const client of clients.values()) {
    if (!room.players.find((p) => p.id === client.id)) continue;
